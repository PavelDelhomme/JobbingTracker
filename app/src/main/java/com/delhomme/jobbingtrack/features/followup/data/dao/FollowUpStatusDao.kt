...
interface FollowUpStatusDao : BaseDao<FollowUpStatusEntity> {
    ...
    @Query("SELECT * FROM follow_up_status WHERE id = :id AND isDeleted = 0 LIMIT 1")
    fun getById(id: String): Flow<FollowUpStatusEntity?>

    @Query("SELECT * FROM follow_up_status WHERE userId = :userId AND isDeleted = 0")
    fun getAllForUser(userId: String): Flow<List<FollowUpStatusEntity>>

    @Query("""
        SELECT * FROM follow_up_status 
        WHERE userId = :userId 
    ...
    @Query("UPDATE follow_up_status SET lastSyncAt = :syncTime WHERE id IN (:ids)")
    override suspend fun updateSyncTimestamp(ids: List<String>, syncTime: Long)

    @Query("SELECT * FROM follow_up_status WHERE id = :id AND userId = :userId LIMIT 1")
    override suspend fun getById(id: String, userId: String): FollowUpStatusEntity?

    @Query("UPDATE follow_up_status SET isDeleted = 1, deletedAt = :timestamp WHERE id = :id AND userId = :userId")
