...
interface CompanyDao : BaseDao<CompanyEntity> {
    ...
    @Query("UPDATE companies SET isArchived = 1, archivedAt = :timestamp WHERE id IN(:ids) AND userId = :userId")
    suspend fun archive(ids: List<String>, userId: String, timestamp: Long = System.currentTimeMillis())

    @Query("UPDATE companies SET isDeleted = 1, deletedAt = :timestamp WHERE id IN(:ids) AND userId = :userId")
    suspend fun softDelete(ids: List<String>, userId: String, timestamp: Long = System.currentTimeMillis())

    @Query("UPDATE companies SET isDeleted = 0, deletedAt = NULL WHERE id IN(:ids) AND userId = :userId")
    ...
    @Query("UPDATE companies SET lastSyncAt = :syncTime WHERE id IN (:ids)")
    ...
    @Query("UPDATE companies SET isDeleted = 1, deletedAt = :timestamp WHERE id = :id AND userId = :userId")
